module Univ = struct
  ...
  (** Create a new embedding. Values packed by a given embedding can only be
      unpacked by the same embedding. *)
  let embed () =
    let r = ref None in
    (* place to store values *)
    let pack a =
      (* pack the 'a value into a new univ cell *)
      let o = Some a in
      fun () -> r := o
    in
    let unpack t =
      (* try to extract the content of a univ cell *)
      r := None;
      t ();
      let a = !r in
      a
...
module type S = sig
  ...
  (** A datalog clause is safe iff all variables in its head also occur in its
      body *)
  ...
  (** This part of the API can be used to avoid building variables yourself.
      Calling [quantify3 f] with call [f] with 3 distinct variables, and [f] can
      use those variables to, for instance, build a clause *)
  ...
  type explanation =
    ...
    | ExtExplanation of string * Univ.t
        (** Explanation for a clause or fact. It is extensible through universal
            types. *)
  ...
  (** Add the clause/fact to the DB as an axiom, updating fixpoint. UnsafeRule
      will be raised if the rule is not safe (see {!check_safe}) *)
  ...
  (** match the given literal with facts of the DB, calling the handler on each
      fact that match *)

  val db_query : db -> literal -> int list -> (symbol list -> unit) -> unit
  (** Like {!db_match}, but the additional int list is used to select bindings
      of variables in the literal. Their bindings, in the same order, are given
      to the callback. *)
  ...
  (** Add a function to be called on new literals. Only one function per symbol
      can be registered. *)
  ...
  (** Explain the given fact by returning a list of facts that imply it under
      the current clauses, or raise Not_found *)

  val db_premises : db -> literal -> clause * literal list
  (** Immediate premises of the fact (ie the facts that resolved with a clause
      to give the literal), plus the clause that has been used. *)
  ...
  module Query : sig
    ...
    (** Given a list of variables, and a list of literals that contain those
        variables, return a set. Each element of the set is an instantiation of
        the variables such that all instantiated literals are facts of the [db].
        [neg] is an optional list of literals that must be false for an
        instantiation to be an answer. This is lazy, and will only be evaluated
        upon calls to {! iter}, {! to_list} or other similar functions. The
        answers will be cached in the set and readily available thereafter. *)
...
module Make (Symbol : SymbolType) : S with type symbol = Symbol.t = struct
  ...
  type subst =
    ...
    | SubstBind of (int * int * term * int * subst)
        (** A substitution is a map from (negative) ints (variables) with
            context to ints with context (variable or symbol) *)

  type 'a bind = 'a * int
  (** A context in which to interpret variables in a literal or clause. The
      context is an offset that is implicitely applied to variables *)
  ...
  let eq_literal t1 t2 =
    let rec check t1 t2 i =
      if i = Array.length t1 then
        ...
        eq_term t1.(i) t2.(i) && check t1 t2 (i + 1)
    in
  ...
  let check_safe clause =
    ...
    and check_body_literal var literal k =
      if k = Array.length literal then
        false
      else if eq_term literal.(k) var then
        true
      else
        check_body_literal var literal (k + 1)
    in
    check_head 1

  (** A fact is a ground clause with empty body *)
  let is_fact clause = Array.length clause = 1 && is_ground clause.(0)

  (** Check whether clauses are (syntactically) equal *)
  let eq_clause r1 r2 =
    let rec check i =
      if i = Array.length r1 then
        true
      else
        eq_literal r1.(i) r2.(i) && check (i + 1)
    in
  ...
  (** Dereference [var] in context [offset] until it is no longer a bound
      variable *)
  ...
  let matching ?(subst = empty_subst) (l1, o1) (l2, o2) =
    if Array.length l1 <> Array.length l2 then
      ...
      and match_pair subst t1 o1' t2 o2' =
        ...
        | Var _, _ -> bind_subst subst t1 o1' t2 o2' (* bind var *)
        | Const _, Var _ -> raise UnifFailure (* cannot bind t2 *)
      in
      match_pairs subst 0
    )

  (** [unify l1 l2] tries to unify [l1] with [l2]. Raise UnifFailure if they do
      not match. *)
  let unify ?(subst = empty_subst) (l1, o1) (l2, o2) =
    if Array.length l1 <> Array.length l2 then
      ...
      and unif_pair subst t1 o1' t2 o2' =
        ...
        | Var i, Var j when i = j && o1' = o2' -> subst
        | Var _, _ -> bind_subst subst t1 o1' t2 o2' (* bind var *)
        | Const _, Var _ -> bind_subst subst t2 o2' t1 o1' (* bind var *)
      in
      unif_pairs subst 0
    )
      ...
      and unif_pair subst t1 o1' t2 o2' =
        ...
        | Const s1, Const s2 ->
          if Symbol.equal s1 s2 then
            ...
            raise UnifFailure
        | Var i, Var j when i = j && o1' = o2' -> subst
        | Var _, Var _ -> bind_subst subst t1 o1' t2 o2' (* bind var *)
        | Const _, Var _ | Var _, Const _ -> raise UnifFailure
      in
      unif_pairs subst 0
    )
  ...
  module MakeIndex (X : Hashtbl.HashedType) : Index with type elt = X.t = struct
    ...
    (** The literal index. It is a trie with, at each node, a hashset of
        elements, plus a map symbol/var -> subtrie. All variables are mapped to
        [Var 0], because the tree is non-perfect. This makes matching slower,
        but consumes less memory. *)
    type t = Node of unit DataSet.t * t TermHashtbl.t
    ...
    let rec copy t =
      ...
      | Node (set, h) ->
        ...
        TermHashtbl.iter (fun k t' -> TermHashtbl.add h' k (copy t')) h;
        Node (set', h')
    ...
    let retrieve_generalizations k acc (t, o_t) (literal, o_lit) =
      ...
      and try_with subtries acc sym i =
        try
          let t' = TermHashtbl.find subtries sym in
          search t' (i + 1) acc
        with Not_found -> acc
      in
      search t 0 acc
      ...
      let rec search t i acc =
        ...
        | Node (set, _), i when i = len ->
          DataSet.fold
            ...
            set acc
        | Node (_, subtries), i ->
          if is_var literal.(i) then (* fold on all subtries *)
            TermHashtbl.fold
      ...
      let rec search t i acc =
        ...
        | Node (_, subtries), i ->
          if is_var literal.(i) then (* fold on all subtries *)
            ...
            (* try both subtrie with same symbol, and subtrie with variable *)
    ...
    let retrieve_renaming k acc (t, o_t) (literal, o_lit) =
      ...
      let rec search t i acc =
        ...
        | Node (_, subtries), i ->
          let c = term_to_char literal.(i) in
          (try
             let t' = TermHashtbl.find subtries c in
             search t' (i + 1) acc
           with Not_found -> acc)
      in
      search t 0 acc
    ...
    (** Fold on all indexed elements *)
    let rec fold k acc t =
      match t with
      | Node (set, subtries) ->
        (* fold on elements at this point *)
        let acc =
          DataSet.fold (fun (lit, elt) () acc -> k acc lit elt) set acc
        in
        (* fold on subtries *)
        TermHashtbl.fold (fun _ subtrie acc -> fold k acc subtrie) subtries acc
  ...
  module ClauseHashtbl = Hashtbl.Make (struct
    type t = clause
  ...
  type db = {
    db_all: explanation ClauseHashtbl.t;
        (** maps all clauses to their explanations *)
    ...
    db_fact_handlers: fact_handler list SymbolHashtbl.t;
        (** map symbol -> fact handlers *)
    ...
    let rec rewrite_lit db_funs lit =
      ...
      | Const s ->
        let lit' =
          ...
          with Not_found -> lit
        in
        if lit == lit' || eq_literal lit lit' then
          lit'
        (* fixpoint *)
        else
          rewrite_lit db_funs lit'
    in
  ...
  let add_clause db clause explanation =
    ...
    (* generate new clauses by resolution *)
      ...
      let call_handler h =
        try h clause.(0)
        with e ->
          Format.eprintf "Datalog: exception while calling handler for %s@."
            (Symbol.to_string s);
          raise e
      in
      (match SymbolHashtbl.find db.db_fact_handlers s with
  ...
  (** Push the item in the queue, and process items in the queue if no other
      function call is already doing it *)
  ...
  (** Add a goal to the DB. The goal is used to trigger backward chaining
      (calling goal handlers that could help solve the goal) *)
  let db_goal db lit = process_items db (`AddGoal lit)

  (** match the given literal with facts of the DB, calling the handler on each
      fact that match (with the corresponding substitution) *)
  ...
  let db_fold k acc db =
    ClauseHashtbl.fold (fun clause _ acc -> k acc clause) db.db_all acc
  ...
  let db_subscribe_fact db symbol handler =
    let l =
      try SymbolHashtbl.find db.db_fact_handlers symbol with Not_found -> []
    in
  ...
  (** Iterate on all current goals *)
  let db_goals db k = GoalIndex.fold (fun () goal () -> k goal) () db.db_goals
  ...
  let db_explain db fact =
    ...
    let rec search clause =
      if ClauseHashtbl.mem explored clause then
        ...
        | Axiom when is_fact clause -> LitHashtbl.replace set clause.(0) ()
        | ExtExplanation _ | Axiom -> ()
  ...
  let db_premises db fact =
    let rec search acc clause =
      let explanation = ClauseHashtbl.find db.db_all clause in
      match explanation with
      | ExtExplanation _ | Axiom -> clause, acc (* no premises *)
      | Resolution (clause, fact) ->
        let acc = fact :: acc in
        search acc clause
    in
  ...
  (** Get all the explanations that explain why this clause is true *)
  let db_explanations db clause = ClauseHashtbl.find_all db.db_all clause
  ...
  module Query = struct
    ...
    and expr =
      | Match of
          literal * int array * int array (* match with literal, then project *)
      | Join of query * query (* join on common variables *)
      | ProjectJoin of
          int array * query * query (* join and project immediately *)
      | Project of int array * query (* project on given columns*)
      | AntiJoin of
          query * query (* tuples of q1 that are not joinable with q2 *)
    ...
    let mk_query expr =
      let q_vars =
        ...
        | Match (_, vars, _) -> vars
        | Join (q1, q2) ->
          if common_vars q1.q_vars q2.q_vars = [||] then
            Array.append q1.q_vars q2.q_vars
          else
            union_vars q1.q_vars q2.q_vars
    ...
    let mk_table vars = { tbl_vars = vars; tbl_rows = RowTable.create 27 }
    let add_table tbl row = RowTable.replace tbl.tbl_rows row ()
    let iter_table tbl k = RowTable.iter (fun row () -> k row) tbl.tbl_rows
    ...
    let rec optimize q =
      ...
      | Project (vars, q') ->
        if vars = q'.q_vars then
          optimize q' (* reord is the identity *)
        else
          mk_query (Project (vars, optimize q'))
    ...
    let select_indexes indexes a = Array.map (fun i -> Array.get a i) indexes
      ...
      | None ->
        let tbl =
          ...
          | Project (vars, q) ->
            ...
            iter_table tbl (fun row ->
                let row' = project indexes row in
                add_table result row');
            result
          | ProjectJoin (vars, q1, q2) -> eval_join ~vars db q1 q2
          | Join (q1, q2) -> eval_join ?vars:None db q1 q2
          | AntiJoin (q1, q2) ->
    ...
    and product tbl1 tbl2 =
      ...
      let tbl = mk_table vars in
      iter_table tbl1 (fun row1 ->
          iter_table tbl2 (fun row2 ->
    ...
    and project_product ~vars tbl1 tbl2 =
      let tbl = mk_table vars in
      let indexes =
        find_indexes vars (Array.append tbl1.tbl_vars tbl2.tbl_vars)
      in
      iter_table tbl1 (fun row1 ->
          iter_table tbl2 (fun row2 ->
              let row = Array.append row1 row2 in
    ...
    and antijoin tbl1 tbl2 =
      ...
      iter_table tbl1 (fun row ->
          let join_items = select_indexes common_indexes row in
          if Hashtbl.mem idx2 join_items then
            ()
          (* drop row *)
          else
            add_table result row);
    ...
    let pp_plan formatter set =
      ...
      let rec pp_q ~top fmt q =
        match q.q_expr with
        | Match (lit, vars, _) ->
          Format.fprintf fmt "match[%a] %a" (pp_array ~sep:"," pp_var) vars
            pp_literal lit
        | Join (q1, q2) ->
          if not top then Format.pp_print_string fmt "(";
          Format.fprintf fmt "%a |><| %a" (pp_q ~top:false) q1 (pp_q ~top:false)
            q2;
          if not top then Format.pp_print_string fmt ")"
        | ProjectJoin (vars, q1, q2) ->
          if not top then Format.pp_print_string fmt "(";
        ...
        | AntiJoin (q1, q2) ->
          Format.fprintf fmt "%a |> %a" (pp_q ~top:false) q1 (pp_q ~top:false)
            q2
        | Project (vars, q) ->
          Format.fprintf fmt "project[%a] %a" (pp_array ~sep:"," pp_var) vars
            (pp_q ~top:false) q
      in
      pp_q ~top:true formatter set.query
  end
end
