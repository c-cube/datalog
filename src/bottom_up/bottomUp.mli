...
(** Datalog is a fragment of first-order logic. It can be viewed as relational
    algebra with deduction/recursion (through {b Horn clauses}).

    A Datalog database contains {b facts} and {b clauses} (or rules). A fact is
    simply a relational atom, like [mother(mary, john)]. A Horn clause is a
    deduction rule, for instance [grandmother(X,Y) :- mother(X,Z), parent(Z,Y)]
...
(** This module is present to allow the user to extend explanations with her own
    types. *)
module Univ : sig
  (** This is largely inspired by
      {{:https://ocaml.janestreet.com/?q=node/18}this thread} *)
...
(** Main module, that exposes datatypes for logic literals and clauses,
    functions to manipulate them, and functions to compute the fixpoint of a set
    of clauses *)
