...
let mk_vartbl () = { vartbl_count = 0; vartbl_tbl = Hashtbl.create 5 }
...
let term_of_ast ~tbl ast =
  match ast with
  | A.Const s | A.Quoted s -> mk_const (StringSymbol.make s)
  | A.Var x -> mk_var (getvar ~tbl x)

let literal_of_ast ?(tbl = mk_vartbl ()) lit =
  match lit with
  | A.Atom (s, args) ->
