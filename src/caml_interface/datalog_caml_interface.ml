...
module Univ = struct
  ...
  let float = new_key ~eq:(fun i j -> i = j) ~print:string_of_float ()
  ...
  let pair a b =
    new_key
      ...
      ~print:(fun (a1, b1) ->
        Printf.sprintf "(%s,%s)" (a.print a1) (b.print b1))
      ()
  ...
  let list a =
    ...
    new_key
      ~eq:(fun l1 l2 ->
        try List.for_all2 a.eq l1 l2 with Invalid_argument _ -> false)
  ...
  let array k =
    ...
    and _hash a =
      ...
      !h
    in
    new_key ~eq:_eq ~hash:_hash ~print:_print_arr ()
end
...
module Rel1 = struct
  ...
  let subset db (n1, _) (n2, _) =
    let x = T.mk_var 0 in
    let c =
      C.mk_clause (T.mk_apply n2 [| x |])
        [ Logic.Lit.mk_pos (T.mk_apply n1 [| x |]) ]
    in
    DB.add_clause db c

  let from_fun db ((n, _k) as rel) f =
    DB.interpret db n (fun t ->
        match get rel t with
        | None -> []
        | Some x ->
          if f x then
            [ C.mk_fact t ]
          (* the fact is true, says [f] *)
          else
            [])

  let add_list db rel l = List.iter (fun t -> DB.add_fact db (make rel t)) l
...
module Rel2 = struct
  ...
  let get (name, k1, k2) t =
    match t with
    | T.Apply (name', [| T.Apply (u1, [||]); T.Apply (u2, [||]) |])
      when Univ.eq name name' ->
      (match Univ.unpack ~key:k1 u1, Univ.unpack ~key:k2 u2 with
  ...
  let add_list db rel l =
    List.iter (fun (x, y) -> DB.add_fact db (make rel x y)) l
...
module Rel3 = struct
  ...
  let create ?(k1 = Univ.new_key ()) ?(k2 = Univ.new_key ())
      ?(k3 = Univ.new_key ()) name =
    of_string name, k1, k2, k3

  let get (name, k1, k2, k3) t =
    match t with
    | T.Apply
        (name', [| T.Apply (u1, [||]); T.Apply (u2, [||]); T.Apply (u3, [||]) |])
      when Univ.eq name name' ->
      (match
         Univ.unpack ~key:k1 u1, Univ.unpack ~key:k2 u2, Univ.unpack ~key:k3 u3
       with
  ...
  let add_list db rel l =
    List.iter (fun (x, y, z) -> DB.add_fact db (make rel x y z)) l
         ...
         let l =
           Array.fold_left
             (fun acc t' ->
               match t' with
               | T.Apply (u, [||]) ->
                 (match Univ.unpack ~key:k u with
                 | None -> raise Exit
                 | Some x -> x :: acc)
               | _ -> raise Exit)
             [] a
...
let add_builtin db =
  let builtin =
    ...
    and _eval goal =
      ...
      | Logic.T.Apply (_, subgoals) ->
        ...
        Array.fold_left
          (fun acc sub ->
            Logic.C.mk_clause goal [ Logic.Lit.mk_pos sub ] :: acc)
          [] subgoals
      | _ -> []
    in
    [
  ...
  let _assertz db goal =
    ...
    | _ -> []
  in
  DB.interpret ~help:"assertz(fact): add fact to the DB" db
    (of_string "assertz") (_assertz db);
  ()
