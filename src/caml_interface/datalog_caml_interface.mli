...
(** {2 Constants with universal types}
    This module is present to allow the user to pack their own types into
    Datalog constants. It is {b NOT thread-safe}. This is largely inspired by
    {{:https://web.archive.org/web/20150907070243/https://blogs.janestreet.com/a-universal-type/}this
     thread} *)
...
module Univ : sig
  ...
  val new_key :
    ...
    unit ->
    'a key
  (** Create a new key. Values packed with a given key can only be unpacked by
      the same key.

      @param print
        a printing function to print the content of a universal value. Default
        just returns "opaque".
...
module Rel1 : sig
  ...
  (** Check whether this term is a "R(t)" with [t] an object packed with the
      appropriate key, and "R" the name of the given relation *)
  ...
  (** [subset db r1 r2] adds to [db] the axiom that [r2(X) :- r1(X)]; in other
      words, [r1] is a subset of [r2] as a relation *)

  val from_fun : Logic.DB.t -> 'a t -> ('a -> bool) -> unit
  (** The given function decides of the given relation (if it returns true for a
      constant, then the relation holds for this constant) *)
...
module Rel2 : sig
  ...
  (** [subset db r1 r2] adds to [db] the axiom that [r2(X,Y) :- r1(X,Y)]; in
      other words, [r1] is a subset of [r2] as a relation *)

  val transitive : Logic.DB.t -> ('a, 'a) t -> unit
  (** Axioms for transitivity are added to the DB *)

  val tc_of : Logic.DB.t -> tc:('a, 'a) t -> ('a, 'a) t -> unit
  (** [tc_of db ~tc r] adds to [db] axioms that make the relation [tc] the
      transitive closure of the relation [r]. *)

  val reflexive : Logic.DB.t -> ('a, 'a) t -> unit
  (** [reflexive db r] makes [r] reflexive in [db], ie for all [X], [r(X,X)]
      holds in [db]. *)
  ...
  (** The given function decides of the given relation (if it returns true for a
      couple of constants, then the relation holds for those constants) *)
...
module Rel3 : sig
  ...
  val from_fun :
    Logic.DB.t -> ('a, 'b, 'c) t -> ('a -> 'b -> 'c -> bool) -> unit
