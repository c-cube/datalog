...
module type CONST = sig
  ...
  (** Special symbol, that will never occur in any user-defined clause or term.
      For strings, this may be the empty string "". *)
end
...
module type S = sig
  ...
  module Subst : sig
    ...
    (** While the term is a variable bound in subst, follow its binding. Returns
        the final term and scope *)

    val create_renaming : unit -> renaming
    val reset_renaming : renaming -> unit

    val rename : renaming:renaming -> T.t -> scope -> T.t
    (** Rename the given variable into a variable that is unique within
        variables known to the given [renaming] *)

    val eval : t -> renaming:renaming -> T.t -> scope -> T.t
    (** Apply the substitution to the term. Free variables are renamed using
        [renaming] *)
  ...
  (** For {!unify} and {!match_}, the optional parameter [oc] is used to enable
      or disable occur-check. It is disabled by default. *)
  ...
  val match_ :
    ?oc:bool -> ?subst:Subst.t -> T.t -> scope -> T.t -> scope -> Subst.t
  (** [match_ a sa b sb] matches the pattern [a] in scope [sa] with term [b] in
      scope [sb].
      @raise UnifFail if it fails *)
  ...
  (** Special version of [alpha_equiv], using distinct scopes for the two terms
      to test, and discarding the result *)
  ...
  (** {2 Special built-in functions}
      ...
      meaning is given by a set of OCaml functions that can evaluate
      applications of the function symbol to arguments.
  ...
  module BuiltinFun : sig
    ...
    type map
    (** Map symbols to builtin functions. Every symbol can only have at most one
        built-in function. *)

    val create : unit -> map

    val add : map -> Const.t -> t -> unit
    (** Interpret the given constant by the given function. The function can
        assume that any term is it given as a parameter has the constant as
        head. *)
  ...
  (** {2 Index}
      ...
      involves finding all data associated with terms that match, or unify with,
      a given term. *)
  ...
  module Index (Data : Hashtbl.HashedType) : sig
    ...
    val generalizations :
      ...
      (Data.t -> Subst.t -> unit) ->
      unit
    (** Retrieve data associated with terms that are a generalization of the
        given query term *)
  ...
  (** {2 Rewriting}
      Rewriting consists in having a set of {b rules}, oriented from left to
      right, that we will write [l -> r] (say "l rewrites to r"). Any term t
      that l matches is {b rewritten} into r by replacing it by sigma(r), where
      sigma(l) = t. *)
  ...
  (** A DB stores facts and clauses, that constitute a logic program. Facts and
  ...
  module DB : sig
    ...
    (** Interpreted predicate. It takes terms which have a given symbol as head,
        and return a list of (safe) clauses that have the same symbol as head,
        and should unify with the query term. *)
    ...
    (** Add an interpreter for the given constant. Goals that start with this
        constant will be given to all registered interpreters, all of which can
        add new clauses. The returned clauses must have the constant as head
        symbol. *)
    ...
    val find_facts :
      ?oc:bool -> t -> scope -> T.t -> scope -> (T.t -> Subst.t -> unit) -> unit
    (** find facts unifying with the given term, and give them along with the
        unifier, to the callback *)

    val find_clauses_head :
      ?oc:bool -> t -> scope -> T.t -> scope -> (C.t -> Subst.t -> unit) -> unit
    (** find clauses whose head unifies with the given term, and give them along
        with the unifier, to the callback *)

    val find_interpretation :
      ?oc:bool -> t -> scope -> T.t -> scope -> (C.t -> Subst.t -> unit) -> unit
    (** Given an interpreted goal, try all interpreters on it, and match the
        query against their heads. Returns clauses whose head unifies with the
        goal, along with the substitution. *)
  end
  ...
  val ask :
    ...
    T.t ->
    T.t list
  (** Returns the answers to a query in a given DB. Additional facts and rules
      can be added in a local scope.
  ...
  val ask_lits :
    ...
    Lit.t list ->
    T.t list
  (** Extension of {! ask}, where the query ranges over the list of variables
      (the term list), all of which must be bound in the list of literals that
      form a constraint.

      [ask_lits db vars lits] queries over variables [vars] with the constraints
      given by [lits].

      Conceptually, the query adds a clause (v1, ..., vn) :- lits, which should
      respect the same safety constraint as other clauses.

      @return
        a list of answers, each of which is a list of terms that map to the
        given list of variables. *)
end

(** {2 Generic implementation} *)

let combine_hash hash i = abs ((hash * 65599) + i)

(** Hash a list. Each element is hashed using [f]. *)
let rec hash_list f h l =
  match l with
  | [] -> h
...
let _array_fold2 f acc a1 a2 =
  if Array.length a1 <> Array.length a2 then
    failwith "_array_fold2: arrays must have same length";
  let acc = ref acc in
  for i = 0 to Array.length a1 - 1 do
    acc := f !acc a1.(i) a2.(i)
  done;
...
module Make (Const : CONST) = struct
  ...
  let _debug k = if !_debug_enabled then _debug_real k
  ...
  module T = struct
    ...
    let rec eq t1 t2 =
      ...
      | Apply (c1, l1), Apply (c2, l2) ->
        ...
        && Const.equal c1 c2 && _array_forall2 eq l1 l2
      | Var _, Apply _ | Apply _, Var _ -> false
    ...
    let rec ground t =
      ...
      | Var _ -> false
      | Apply (_, [||]) -> true
      | Apply (_, args) -> _ground_arr args 0

    and _ground_arr a i =
      if i = Array.length a then
        true
      else
        ground a.(i) && _ground_arr a (i + 1)

    let vars t =
      let rec _gather acc t =
        match t with
        | Var i when _var_present acc i -> acc
        | Var i -> i :: acc
        | Apply (_, [||]) -> acc
        | Apply (_, args) -> Array.fold_left _gather acc args
      and _var_present l i =
        match l with
        | [] -> false
  ...
  module Lit = struct
    ...
    let mk_aggr ~left ~constructor ~var ~guard =
      LitAggr { left; constructor; var; guard }

    let eq lit1 lit2 =
      match lit1, lit2 with
      | LitPos t1, LitPos t2 | LitNeg t1, LitNeg t2 -> T.eq t1 t2
      | LitAggr a1, LitAggr a2 ->
        T.eq a1.left a2.left && T.eq a1.var a2.var && T.eq a1.guard a2.guard
    ...
    let fmap f lit =
      ...
      | LitNeg t -> LitNeg (f t)
      | LitAggr a ->
        LitAggr { a with left = f a.left; var = f a.var; guard = f a.guard }
    ...
    let to_string lit =
      ...
      | LitAggr a ->
        Printf.sprintf "%s := %s %s : %s" (T.to_string a.left)
          (Const.to_string a.constructor)
          (T.to_string a.var) (T.to_string a.guard)
  ...
  module C = struct
    ...
    let _safe_clause _head _body = true (* TODO *)

    let mk_clause head body =
      if _safe_clause head body then
        { head; body }
      else
        raise Unsafe
    ...
    let pp oc c = output_string oc (to_string c)
  ...
  module Subst = struct
    ...
    let deref subst t scope =
      let rec search subst v i scope =
        match subst with
        | Nil -> v, scope
    ...
    let rec eval subst ~renaming t scope =
      ...
      | T.Apply (_c, [||]) -> t
      | T.Apply (c, args) ->
        let args' = Array.map (fun t' -> eval subst ~renaming t' scope) args in
        T.mk_apply c args'
    ...
    let fmt out (s : t) =
      let rec aux out = function
        ...
        | Bind (x1, sc1, t2, sc2, tl) ->
          Format.fprintf out "@[X%d[%d] ->@ %a[%d]@],@ %a" x1 sc1 T.fmt t2 sc2
            aux tl
      in
  ...
  let rec _occur_check subst v sc_v t sc_t =
    match t with
  ...
  let _lit_alpha_equiv ~subst lit1 sc1 lit2 sc2 =
    match lit1, lit2 with
    | Lit.LitPos t1, Lit.LitPos t2 | Lit.LitNeg t1, Lit.LitNeg t2 ->
  ...
  module BuiltinFun = struct
    ...
    let add map c f = ConstTbl.replace map c f
    let add_list map l = List.iter (fun (c, f) -> add map c f) l
  ...
  (** Functor that allows fast retrieval of sets of values for the given
      {! Data} type, by unification or matching with a term. This is a kind of
      fingerprint indexing, but only at the very first level of subterms. *)
  module Index (Data : Hashtbl.HashedType) = struct
    ...
    module TermDataTbl = Hashtbl.Make (struct
      ...
      let equal (t1, d1) (t2, d2) = are_alpha_equiv t1 t2 && Data.equal d1 d2
      let hash (t, d) = combine_hash (T.hash_novar t) (Data.hash d)
    end)
    ...
    let create size = { sub = ConstTbl.create size; var = None; data = None }
    let empty () = create 23
    ...
    let add idx t data =
      ...
      let rec add tree i =
        if i = Array.length arr then (
          ...
          | Const s ->
            let tree' =
              try ConstTbl.find tree.sub s with Not_found -> create 5
            in
            let tree' = add tree' (i + 1) in
      ...
      let rec remove tree i =
        if i = Array.length arr then (
          ...
          | Some set ->
            ...
            if TermDataTbl.length set = 0 then tree.data <- None;
            (* remove data set *)
            tree
        ) else (
          match arr.(i) with
      ...
      let rec iter tree i =
        if i = Array.length arr then (
          ...
          | Var ->
            ...
            ConstTbl.iter (fun _ tree' -> iter tree' (i + 1)) tree.sub
          | Const s ->
    ...
    let rec iter t f =
      ...
      (match t.data with
      | None -> ()
      | Some set -> TermDataTbl.iter (fun (t, data) () -> f t data) set);
      ConstTbl.iter (fun _ t' -> iter t' f) t.sub
    ...
    let rec size t =
      ...
      in
      ConstTbl.fold (fun _ t' s -> size t' + s) t.sub s
  end
  ...
  module DB = struct
    ...
    let add_fact db t = db.facts <- TermIndex.add db.facts t t
    ...
    let interpret ?help db c inter =
      ...
      with Not_found -> ConstTbl.add db.interpreters c [ inter ]

    let interpret_list db l =
      List.iter (fun (c, help, i) -> interpret ~help db c i) l

    let is_interpreted db c = ConstTbl.mem db.interpreters c
  ...
  module Query = struct
    ...
    and goal_entry = {
      ...
      mutable exit_fired: bool;
          (* exit frame processed, pending completion (idempotence) *)
    }
    (** Root of the proof forest *)

    (** In a goal entry, [poss] and [negs] are other goals that depend on this
        given goal. IT's the reverse dependency graph. When an answer is added
        to this goal, it's also propagated to waiters. *)
    ...
    let all_deps_complete (g : goal_entry) : bool =
      try
        T.Tbl.iter
          (fun _ dep -> if not dep.complete then raise_notrace Exit)
          g.dep_goals;
        true
      with Exit -> false
    ...
    let _iter_answers k node = T.Tbl.iter (fun t () -> k t) node.answers
    ...
    let rec slg_main ~query =
      ...
      | Exit (goal_entry, stack') ->
        query.stack <- stack';
        (* close goal entry *)
        if not goal_entry.complete then slg_exit ~query goal_entry;
        slg_main ~query
    ...
    and slg_solve ~query goal =
      ...
      try TVariantTbl.find query.forest goal
      with Not_found ->
    ...
    and slg_newclause ~query goal_entry clause =
      _debug (fun k ->
          k "slg_newclause with %a and clause %a" T.fmt goal_entry.goal C.fmt
            clause);
      match clause.C.body with
      | [] ->
    ...
    and slg_answer ~query goal_entry ans =
      ...
      assert (T.ground ans);
      if (not goal_entry.complete) && not (T.Tbl.mem goal_entry.answers ans)
      then (
        (* new answer! *)
    ...
    and slg_positive ~query goal_entry clause subgoal =
      _debug (fun k ->
          k "slg_positive %a with clause %a, subgoal %a" T.fmt goal_entry.goal
            C.fmt clause T.fmt subgoal);
      ...
      if
        (not subgoal_entry.complete)
        && not (T.Tbl.mem (get_dep_goals goal_entry) subgoal)
      then (
    ...
    and slg_negative ~query goal_entry clause neg_subgoal =
      _debug (fun k ->
          k "slg_negative %a with clause %a, neg_subgoal %a" T.fmt
            goal_entry.goal C.fmt clause T.fmt neg_subgoal);
      let subgoal_entry = slg_solve ~query neg_subgoal in
      if T.Tbl.length subgoal_entry.answers = 0 then
        if subgoal_entry.complete then
    ...
    and slg_aggregate ~query goal_entry clause subgoal =
      _debug (fun k ->
          k "slg_aggregate %a with clause %a, subgoal %a" T.fmt goal_entry.goal
            C.fmt clause T.fmt subgoal);
    ...
    and slg_complete_aggregate ~query goal_entry clause answers =
      _debug (fun k ->
          k "slg_complete_aggregate %a with %a (%d ans)" T.fmt goal_entry.goal
            C.fmt clause (T.Tbl.length answers));
      ...
      T.Tbl.iter
        (fun t res_l ->
          ...
          _debug (fun k ->
              k "@[slg_aggr.group: t: %a,@ aggr_t: %a,@ eval-into: %a@]" T.fmt t
                T.fmt aggr_t_raw T.fmt aggr_t);
          try
            ...
            let answer = Subst.eval subst ~renaming goal_entry.goal 0 in
            _debug (fun k ->
                k "@[<2>slg_aggr.answer: %a@ subst: %a@]" T.fmt answer Subst.fmt
                  subst);
            (* add answer *)
    ...
    and try_complete ~query goal_entry =
      if
        goal_entry.exit_fired && (not goal_entry.complete)
    ...
    and slg_complete ~query goal_entry =
      ...
      if T.Tbl.length goal_entry.answers = 0 then
      ...
      List.iter
        (fun (waiter, _) ->
          if not (T.Tbl.mem seen waiter.goal) then (
  ...
  let ask ?(oc = false) ?(with_rules = []) ?(with_facts = []) db lit =
    ...
    let db =
      match with_rules, with_facts with
      | [], [] -> db
      | _ ->
...
module MakeParse (C : PARSABLE_CONST) (TD : S with type Const.t = C.t) = struct
  ...
  let clause_of_ast ?(ctx = Hashtbl.create 3) c =
    match c with
    | head, body ->
  ...
  let parse_file f =
    ...
    try
      let res =
        _parse ~msg:("error while parsing " ^ f) (Lexing.from_channel ic)
      in
      close_in ic;
      res
    with e ->
  ...
  let parse_string s =
    _parse ~msg:"error while parsing string" (Lexing.from_string s)
...
module Default = struct
  ...
  let default_interpreters =
    let _less goal =
      ...
      match goal with
      | T.Apply (_, [| T.Apply (a, [||]); T.Apply (b, [||]) |]) when a < b ->
        [ C.mk_fact goal ]
      | _ -> []
    and _lesseq goal =
      match goal with
    ...
    and _print goal =
      (match goal with
      | T.Apply (_, [| a |]) when T.ground a -> Printf.printf "> %a\n" T.pp a
      | _ -> ());
      [ C.mk_fact goal ]
    ...
    [
      ...
      ( String "eval",
        "eval(*goals): add eval(goals) :- g for each g in goals",
        _eval );
    ]
